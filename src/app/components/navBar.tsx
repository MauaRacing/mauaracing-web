      ...
      <div className={`flex ${navMenu ? "flex-col" : "flex-row"} place-content-center`}>
        <div className="group relative" onMouseOver={() => setOpenAbout(true)} onMouseLeave={() => setOpenAbout(false)}>
          ...
          <dialog open={openAbout} className={`${dialogColor} rounded-md min-w-44 w-10/12 lg:absolute relative`}>
            <div className={`flex flex-col justify-center grow pt-1 px-2 ${fontColor}`}>
              <Link href={"/about"} className="border-b md:hidden">{dict.navbar.firstGroup.title}</Link>
              <Link href={"/about-fsae"} className="border-b ">{dict.navbar.firstGroup.firstOption}</Link>
              <Link href={"/our-team"} className="border-b ">{dict.navbar.firstGroup.secondOption}</Link>
              <Link href={"/sponsors"} className="">{dict.navbar.firstGroup.thirdOption}</Link>
            </div>
          </dialog>
        </div>
        <Link href={"/cars"} className="px-4 font-semibold text-lg hover:border-t-2 border-red-500 ">{dict.navbar.secondGroup.title}</Link>
        <Link href={"/gallery"} className="px-4 font-semibold text-lg hover:border-t-2 border-red-500">{dict.navbar.thirdGroup.title}</Link>
        <div className="group relative" onMouseOver={() => setOpenMedia(true)} onMouseLeave={() => setOpenMedia(false)}>
          ...
          <dialog open={openMedia} className={`${dialogColor} rounded-md min-w-32 w-10/12 lg:absolute relative`}>
            <div className={`flex flex-col justify-center grow pt-1 px-2 ${fontColor} text-nowrap`}>
              <Link href={"/"} className="border-b md:hidden">{dict.navbar.fourthGroup.title}</Link>
              <Link href={"/"} className="border-b ">{dict.navbar.fourthGroup.firstOption}</Link>
              <Link href={"/"} className="">{dict.navbar.fourthGroup.secondOption}</Link>
            </div>
          </dialog>
        </div>
        <div className="group relative" onMouseOver={() => setOpenGetInvolved(true)} onMouseLeave={() => setOpenGetInvolved(false)}>
          ...
          <dialog open={openGetInvolved} className={`${dialogColor} rounded-md min-w-44 w-10/12 lg:absolute relative`}>
            <div className={`flex flex-col justify-center grow pt-1 px-2 ${fontColor} text-nowrap`}>
              <Link href={"/"} className="border-b md:hidden">{dict.navbar.fifthGroup.title}</Link>
              <Link href={"/"} className="border-b">{dict.navbar.fifthGroup.firstOption}</Link>
              <Link href={"/"} className="">{dict.navbar.fifthGroup.secondOption}</Link>
            </div>
          </dialog>
        </div>
        <Link href={"/contact"} className="px-4 font-semibold text-lg hover:border-t-2 border-red-500">{dict.navbar.sixthGroup.title}</Link>
      </div>
    </nav>
  )
}